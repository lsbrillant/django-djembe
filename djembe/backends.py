...
class EncryptingBackendMixin(object):
    ...
    payload_mime_headers = ["Content-Disposition", "Content-Type", "Mime-Version"]
    ...
    def get_sender_identity(self, address):
        ...
        if address:
            sender_identities = Identity.objects.filter(address=address).exclude(key="")
    ...
    def send(self, email_message):
        ...
        encrypting_identities, encrypting_recipients, plaintext_recipients = self.analyze_recipients(
            email_message
        )
        ...
        if encrypting_identities:
            try:
                encrypted_message = self.encrypt(
                    sender_address, encrypting_identities, message
                )

                self.deliver(
                    sender_address, encrypting_recipients, encrypted_message.as_string()
                )
                sent += 1
            except:
                if self.fail_silently is False:
    ...
    def sign(self, sender_identity, message):
        ...
        signing_cert = BIO.MemoryBuffer(sender_identity.certificate.encode("UTF-8"))
        signing_key = BIO.MemoryBuffer(sender_identity.key.encode("UTF-8"))
        ...
        pkcs7_signed_data = s.sign(
            BIO.MemoryBuffer(content_to_sign.encode("UTF-8")),
        ...
        signed_message = email.message_from_string(pkcs7_string.decode("UTF-8"))

        message.set_payload(signed_message.get_payload())

        for header, value in list(signed_message.items()):
            del message[header]
            message[header] = value
...
class EncryptingSMTPBackend(EncryptingBackendMixin, smtp.EmailBackend):
    ...
    def deliver(self, sender_address, recipients, message):
        ...
        """
        self.logger.info(
            "Delivering message from %s to %s" % (sender_address, recipients)
        )
        return self.connection.sendmail(sender_address, recipients, message)
...
class EncryptingTestBackend(EncryptingBackendMixin, base.BaseEmailBackend):
    ...
    def deliver(self, sender_address, recipients, message):
        ...
        # poison pill #1
        if (
            sender_address == "breakerbreaker@example.com"
            and "recipient1@example.com" in recipients
        ):
        ...
        self.messages.append(
            {"sender": sender_address, "recipients": recipients, "message": message}
        )
